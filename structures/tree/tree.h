...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::deepCopy()
	{
        ...
        int processedSonsCount = 0;

        for (int i = 0; processedSonsCount < degree(); i++) {
	...
	{
		if (parent_ == nullptr) {
            throw std::logic_error("Node doesn't have a parent!");
		}
	...
	inline bool Tree<T>::equalsTree(Tree<T>* other)
	{
        ...
        //alebo Iterator::begin
        auto iteratorOther = other->begin();
        auto endIteratorThis = this->end();

        while (iteratorThis != endIteratorThis) {
	...
	inline void Tree<T>::PreOrderTreeIterator::populatePath(TreeNode<T>* current)
	{
        if (current != nullptr) {
            ...
            for (int i = 0; processedSonsCount < current->degree(); i++) {
